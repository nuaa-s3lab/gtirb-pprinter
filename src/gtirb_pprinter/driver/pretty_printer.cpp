...
std::string cur_name, cur_asm;

class py_asm_module {
  std::vector<std::string> module_names;
  std::vector<std::string> asm_values;
public:
  long unsigned size() {
    return module_names.size();
  }
  void get_name(long unsigned index) {
    if (index > module_names.size()) return;
    cur_name = module_names[index];
  }
  void get_asm(long unsigned index) {
    if (index > module_names.size()) return;
    cur_asm = asm_values[index];
  }
  void clear() {
    module_names.clear();
    asm_values.clear();
  ...
  void add_asm(std::string asm_str) {
    asm_values.emplace_back(asm_str);
  }
};

py_asm_module asm_map;

int get_lib(int argc, char** argv, std::string in_gtirb) {
  asm_map.clear();
  gtirb_layout::registerAuxDataTypes();
  gtirb_pprint::registerAuxDataTypes();
  gtirb_pprint::registerPrettyPrinters();
  ...
  // QRZ notes: change here for input
  std::stringstream gtirb_ss(in_gtirb, std::ios::in | std::ios::binary);
  ...
  if (gtirb::ErrorOr<gtirb::IR*> iOrE = gtirb::IR::load(ctx, gtirb_ss))
    ir = *iOrE;
  ...
  for (auto& MP : Modules) {
    ...
    asm_map.add_name(M.getName());
    asm_map.add_asm(myString);
    LOG_INFO << "ss len: " << myString.length() << "\n";
...
extern "C" {
  long unsigned get_asm_lib_size() {
    return asm_map.size();
  }
  const char* get_name(int index) {
    asm_map.get_name(index);
    return cur_name.c_str();
  }
  const char* get_asm(int index) {
    asm_map.get_asm(index);
    return cur_asm.c_str();
  }
  long unsigned get_asm_len(int index) {
    asm_map.get_asm(index);
    return cur_asm.length();
  }
  int py_get_lib(char *module_name, char *gtirb_str, long unsigned length) {
    std::string in_gtirb(gtirb_str, length);
    int argc = 4;
    char arg0[] = "gtirb-pprinter";
    char arg2[] = "--asm";
    char arg3[] = "base64.s";
    char *argv[64] = {
      {arg0},
    ...
    return get_lib(argc, argv, in_gtirb);
  }
}
